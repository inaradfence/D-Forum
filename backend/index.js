...
// https://h-forum.vercel.app
app.use(express.json());
// CORS configuration
const allowedOrigins = ["http://localhost:3000", "https://h-forum.vercel.app","http://localhost:3001"];
app.use(
  cors({
    ...
    credentials: true, // Change this if you need to handle credentials
  })
);

app.get("/", (req, res) => {
  res.send(`API is running on the ${PORT} Port. Current Time is ${new Date().toString()}`);
});
...
// removed the origin single link to array
